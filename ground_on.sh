...
if [ ! -z "$lo" ];then 
  ...
  echo $! >> /tmp/ground.pid
  $HOME_CMP/wfb_tx -K $HOME_CMP/drone.key -p 4 -u 14800 -k 1 -n 2 $WLAN_CMP > /dev/null 2>&1 &
  echo $! > /tmp/ground.pid
  $HOME_CMP/wfb_rx -K $HOME_CMP/drone.key -p 5 -u 14801 -c 127.0.0.1 -k 1 -n 2 $WLAN_CMP > /dev/null 2>&1 &
  echo $! > /tmp/ground.pid
  #
