...
if __name__ == '__main__':
    ...
    pipeline = Gst.parse_launch ("nvarguscamerasrc bufapi-version=1 ! 'video/x-raw(memory:NVMM),width="+str(WIDTH)+",height="+str(HEIGHT)+",framerate="+str(FPS)+"/1' ! mx.sink_0 nvstreammux width="+str(WIDTH)+" height="+str(HEIGHT)+" batch-size=1 name=mx ! nvinfer config-file-path="deepstream-rtsp.cfg" ! nvvideoconvert ! nvdsosd ! nvvideoconvert ! 'video/x-raw(memory:NVMM), format=I420' ! nvv4l2h264enc bitrate=4000000 preset-level=1 insert-sps-pps=1 bufapi-version=1 ! rtph264pay ! udpsink host="192.168.3.1" port=5600 async=False sync=1")
